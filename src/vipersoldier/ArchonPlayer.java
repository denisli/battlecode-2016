			...
			while (true) {
				...
				if (!escape) {
					if (adjNeutralRobots.length > 0) {
						// if there is a neutral robot adjacent, activate it or
					...
					if (Movement.getToAdjParts(rc)) {
						...
						if (toheal == false && rc.isCoreReady()) {
							...
							if (rc.hasBuildRequirements(RobotType.SCOUT) && rc.isCoreReady() && turnNum > 150
									&& turnNum % 150 >= 0 && turnNum % 150 <= 33 && turnNum < 900) {
								Direction dirToBuild = RobotPlayer.directions[rand.nextInt(8)];
								for (int i = 0; i < 8; i++) {
									// If possible, build in this direction
									if (rc.canBuild(dirToBuild, RobotType.SCOUT)) {
							...
							// if - consecutive turns passed and it didnt
							// receive a scout signal, then build a scout for
							// after turn 900
							if (rc.hasBuildRequirements(RobotType.SCOUT) && rc.isCoreReady() && turnNum > 900
									&& conseqNoSignal >= 50) {
								Direction dirToBuild = RobotPlayer.directions[rand.nextInt(8)];
								for (int i = 0; i < 8; i++) {
									// If possible, build in this direction
							...
							// rc.isCoreReady() && !built && numNearbyGuards <
							// 1) {
							// Direction dirToBuild =
							// RobotPlayer.directions[rand.nextInt(8)];
							// for (int i = 0; i < 8; i++) {
							// // If possible, build in this direction
							...
							// }
							// }
							// if (turnNum%300 > 0 && turnNum%300 <100 &&
							// turnNum>800) { //turn conditions to build viper
							...
							// past turn 500, start building turrets with 1/3
							// chance
